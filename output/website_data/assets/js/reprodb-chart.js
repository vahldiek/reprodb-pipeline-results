 ...
 * styling, no logarithmic scales.
 *
 * Dark-mode aware: reads data-theme attribute and re-renders on change.
 */
(function() {
  'use strict';
  ...
  // --- Theme palettes ---
  var LIGHT = {
    text: '#222', textSec: '#444', tick: '#666',
    bg: '#fff', border: '#d0d7de', grid: '#eef0f3',
    crosshair: '#888', pointStroke: '#fff'
  };
  var DARK = {
    text: '#d6d9dc', textSec: '#a0a4aa', tick: '#8b949e',
    bg: '#161b22', border: '#30363d', grid: '#21262d',
    crosshair: '#8b949e', pointStroke: '#161b22'
  };
  function isDark() {
  ...
  Chart.prototype.destroy = function() {
    ...
    var idx = Chart._all.indexOf(this);
  ...
  Chart.prototype._render = function() {
    ...
    function drawYAxis(range, axisId, side, titleText) {
      ...
      range.ticks.forEach(function(v) {
        ...
        el('line', {
          ...
          y1: y, y2: y, stroke: T().tick, 'stroke-width': 1
        }, svg);
        // Label
        var tx = el('text', {
          x: side === 'left' ? x - 6 : x + 6, y: y + 3,
          'text-anchor': side === 'left' ? 'end' : 'start',
          style: FONT, fill: T().textSec
        }, svg);
        ...
        if (pr > 0) {
          for (var k5 = 0; k5 < pts.length; k5++) {
            ...
            el('circle', {
              ...
              fill: stroke, stroke: T().pointStroke, 'stroke-width': 0.5
            }, svg);
          }
        }
      });
    ...
    if (legendVisible) {
      ...
      items.forEach(function(it) {
        ...
        var lt = el('text', { x: lx + 14, y: ly, style: FONT, fill: T().text }, svg);
  ...
  function attachLineTooltip(svg, wrap, datasets, dsPoints, labels, padLeft, padTop, plotW, plotH, chart) {
    ...
    var crosshair = el('line', {
      ...
      stroke: T().crosshair, 'stroke-width': 1, 'stroke-dasharray': '3,3', visibility: 'hidden'
    }, svg);
    function onMove(evt) {
  ...
  function reRenderAll() {
    Chart._all.slice().forEach(function(c) { try { c.update(); } catch(e) {} });
  }
  var _obs = new MutationObserver(function(muts) {
    for (var i = 0; i < muts.length; i++) {
      if (muts[i].attributeName === 'data-theme') { reRenderAll(); break; }
    }
  });
  _obs.observe(document.documentElement, { attributes: true });
